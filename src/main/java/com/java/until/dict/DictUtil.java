...
public class DictUtil {
	...
	public static Map<String, List<InitDictDto>> getDictMapping() {
		...
		Map<String, List<InitDictDto>> dictAreaMapping = (Map<String, List<InitDictDto>>) CacheUntil
				.get(RedisCacheEmun.DICT_CACHE, CacheUntil.DICT_AREA_ITEM, Map.class);
		if (null == dictMapping) {
		...
		dictMapping.putAll(dictAreaMapping);
		// 判断 如果 类型不正确，则转换
